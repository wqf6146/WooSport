...
//            if (!DaemonEnv.sInitialized) return new ArrayList<>();
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                PowerManager pm = (PowerManager) App.newInstance().getSystemService(Context.POWER_SERVICE);
                boolean ignoringBatteryOptimizations = pm.isIgnoringBatteryOptimizations(App.newInstance().getPackageName());
                if (!ignoringBatteryOptimizations) {
                    Intent dozeIntent = new Intent(Settings.ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS);
                    dozeIntent.setData(Uri.parse("package:" + App.newInstance().getPackageName()));
            ...
            //三星 5.0/5.1 自启动应用程序管理
            Intent samsungLIntent = App.newInstance().getPackageManager().getLaunchIntentForPackage("com.samsung.android.sm");
            ...
            meizuIntent.putExtra("packageName", App.newInstance().getPackageName());
...
//            if (!DaemonEnv.sInitialized) return "";
            ...
            ApplicationInfo ai;
            try {
                pm = App.newInstance().getPackageManager();
                ai = pm.getApplicationInfo(App.newInstance().getPackageName(), 0);
                sApplicationName = pm.getApplicationLabel(ai).toString();
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
                sApplicationName = App.newInstance().getPackageName();
            }
        }
    ...
    public static void whiteListMatters(final Activity a, String reason) {
        ...
        SpCache spCache = new SpCache(a,WooConfig.USER.LOGINBEAN);
        Object intenttag = spCache.get(WooConfig.USER.INTENTTAG,false);
        if (intenttag !=null && (Boolean) intenttag == true)
            return;
...
//        SpCache spCache = new SpCache(a,WooConfig.USER.LOGINBEAN);
        spCache.put(WooConfig.USER.INTENTTAG,true);
    }
...
//        if (!DaemonEnv.sInitialized) return false;
        PackageManager pm = App.newInstance().getPackageManager();
